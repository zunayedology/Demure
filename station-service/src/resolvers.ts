...
export const resolvers = {
  ...
  Mutation: {
    createStation: async (_: any, { name, latitude, longitude, address }: { name: string, latitude: number, longitude: number, address: string }) => {
      const result = await session.run(
    ...
    updateStation: async (_: any, { id, name, latitude, longitude, address }: { id: string, name?: string, latitude?: number, longitude?: number, address?: string }) => {
      const result = await session.run(
